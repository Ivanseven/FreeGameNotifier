import {
  ...
  lastRefreshedDateStorageKey,
  lastClaimedDateStorageKey,
...
function updateLastClaimDateText() {
  ...
  chrome.storage.sync.get([lastClaimedDateStorageKey]).then((result) => {
    if (result.lastClaimedEpochSecs > 0) {
      lastClaimedDateText = dateToLocale(
        epochSecsToDate(result.lastClaimedEpochSecs)
      )
    }
...
function updateLastRefreshedDateText() {
  ...
  chrome.storage.sync.get([lastRefreshedDateStorageKey]).then((result) => {
    if (result.lastRefreshedEpochSecs > 0) {
      lastRefreshedDateText = dateToLocale(
        epochSecsToDate(result.lastRefreshedEpochSecs)
      )
    }
    document.querySelector("#lastRefreshedText").innerText =
      lastRefreshedDateText
  })
}
